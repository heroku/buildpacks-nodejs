...
pub(crate) fn install_npm(
...
) -> Result<Print<SubBullet<Stderr>>, libcnb::Error<NodejsBuildpackError>> {
    ...
    match npm_engine_layer.state {
        ...
        LayerState::Empty { ref cause } => {
            ...
            logger = remove_existing_npm_installation(&npm_cli_script, env, logger)?;
            logger = install_npm_package(&npm_cli_script, env, &downloaded_package_path, logger)?;
        }
    }
...
impl From<NpmEngineLayerError> for libcnb::Error<NodejsBuildpackError> {
