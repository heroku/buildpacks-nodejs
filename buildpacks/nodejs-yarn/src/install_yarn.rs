...
pub(crate) fn install_yarn(
    ...
    match dist_layer.state {
        LayerState::Restored { .. } => {
            log_info(format!("Reusing yarn {}", release.version));
        }
        LayerState::Empty { .. } => {
