...
use heroku_nodejs_utils::error_handling::{
    ErrorMessage, ErrorMessageBuilder, ErrorType, SuggestRetryBuild, SuggestSubmitIssue,
};
use indoc::formatdoc;
...
pub(crate) fn on_pnpm_engine_buildpack_error(error: PnpmEngineBuildpackError) -> ErrorMessage {
    match error {
...
mod tests {
    ...
    fn assert_error_snapshot(error: impl Into<PnpmEngineBuildpackError>) {
        let error_message = strip_ansi(on_pnpm_engine_buildpack_error(error.into()).to_string());
