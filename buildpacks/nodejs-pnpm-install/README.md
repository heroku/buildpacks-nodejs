...
This buildpack will install and cacde `package.json` and `package-lock.json`
...
- Downloads, stores, and hard links `package.json` and `package-lock.json`
...
This buildpack's `bin/detect` will only pass if a `package-lock.json` exists in the
