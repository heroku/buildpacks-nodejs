# Heroku Cloud Native Buildpacks: Node.js Functions

[![Cloud Native Buildpacks Registry: heroku/nodejs-function][registry-badge]][registry-url]
[![CI on Github Actions: heroku/nodejs-function][ci-badge]][ci-url]

> [!IMPORTANT]
> This is a long-term maintenance branch for the Node.js CNBs that are used with the end-of-life Salesforce Functions product.

### Making changes for Node.js Functions

* Create a new branch from the `sf-functions` branch.
* Make the necessary changes and create a PR targeted to the `sf-functions` branch.
* Once CI checks pass and the PR has been approved, merge it.
* Prepare a new release of the `heroku/nodejs-function-invoker`, `heroku/nodejs-npm`, and
  `heroku/nodejs-function` buildpacks using the [Prepare Release](https://github.com/heroku/buildpacks-nodejs/actions/workflows/prepare-release.yml)
  workflow and run it with the following parameters:

  Use workflow from
  : Branch: `sf-functions`

  Bump
  : patch | minor | major (choose one)

* Make sure the prepared release is targeted to the `sf-functions` branch. If it looks good, merge it.
* Publish the release using the [Release](https://github.com/heroku/buildpacks-nodejs/actions/workflows/prepare-release.yml)
  workflow and run it with the following parameters:

  Use workflow from
  : Branch: `sf-functions`

  Execute the release workflow but skip any steps that publish (for testing purposes)
  : (unchecked)

* This will publish the `heroku/nodejs-function-invoker`, `heroku/nodejs-npm`, and
  `heroku/nodejs-function` buildpacks to Docker, the CNB registry, and as a GitHub release with
  a suffix of `-functions`.
* It will also create a PR against https://github.com/heroku/cnb-builder-images for the
  `salesforce-functions` builder only.
  This PR must be merged to finalize the release.

[ci-badge]: https://github.com/heroku/buildpacks-nodejs/actions/workflows/ci.yml/badge.svg?branch=sf-functions

[ci-url]: https://github.com/heroku/languages-github-actions/blob/sf-functions/.github/workflows/ci.yml

[registry-badge]: https://img.shields.io/badge/dynamic/json?url=https://registry.buildpacks.io/api/v1/buildpacks/heroku/nodejs-function&label=version&query=$.latest.version&color=DF0A6B&logo=data:image/png;base64,&labelColor=white

[registry-url]: https://registry.buildpacks.io/buildpacks/heroku/nodejs-function
